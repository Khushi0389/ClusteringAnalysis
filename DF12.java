...
public class DF12 extends JFrame implements ActionListener {
    String str = "";
    FileReader fr = null;
    ...
    public void performKMedoids() {
        ...
        clus = new cluster_data[k];
        for (int i = 0; i < k; i++) {
            clus[i] = new cluster_data();
            clus[i].color = getRandomColor();
        }
        ...
        for (int i = 0; i < k; i++) {
            int randomIndex = random.nextInt(data_m.length);
            while (medoids.contains(randomIndex)) {
                randomIndex = random.nextInt(data_m.length);
            }
            medoids.add(randomIndex);
            clus[i].id = i;
            clus[i].center_lat = data_m[randomIndex].nor_lat;
            clus[i].center_lon = data_m[randomIndex].nor_lon;
            clus[i].center_dep = data_m[randomIndex].nor_dep;
            clus[i].center_mag = data_m[randomIndex].nor_mag;
        }

        // Assign data points to clusters
        boolean converged = false;
        while (!converged) {
            // Reset cluster membership and core count
            for (int i = 0; i < data_m.length; i++) {
                data_m[i].cl_id = -1;
                data_m[i].core_count = -1;
            }
            ...
            for (int i = 0; i < data_m.length; i++) {
                ...
                int nearestMedoid = -1;
                for (int j = 0; j < k; j++) {
                    double distance = calculateDistance(data_m[i], clus[j]);
                    if (distance < minDistance) {
                        minDistance = distance;
                        nearestMedoid = j;
                    }
                }
                data_m[i].cl_id = nearestMedoid;
            }
            ...
            for (int i = 0; i < k; i++) {
                ...
                int newMedoid = currentMedoid;
                for (int j = 0; j < data_m.length; j++) {
                    if (data_m[j].cl_id == i) {
                        double cost = calculateCost(j, i);
                        if (cost < minCost) {
                            minCost = cost;
                            newMedoid = j;
                        }
                ...
                if (newMedoid != currentMedoid) {
                    medoids.set(i, newMedoid);
                    clus[i].center_lat = data_m[newMedoid].nor_lat;
                    clus[i].center_lon = data_m[newMedoid].nor_lon;
                    clus[i].center_dep = data_m[newMedoid].nor_dep;
                    clus[i].center_mag = data_m[newMedoid].nor_mag;
                    medoidsChanged = true;
                }
            }
    ...
    private double calculateCost(int pointIndex, int clusterIndex) {
        // Calculate the total cost of assigning a new medoid
        double cost = 0.0;
        for (int i = 0; i < data_m.length; i++) {
            if (data_m[i].cl_id == clusterIndex) {
                cost += calculateDistance(data_m[i], clus[clusterIndex]);
            }
        }
